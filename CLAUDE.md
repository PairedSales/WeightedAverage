...
- `lib/chartRasterExport.ts` wraps `html-to-image` (`toCanvas`/`toPng`) with shared options (white background, `skipFonts` + `cacheBust` for static-host reliability). Exports are WYSIWYG: `getExportPixelRatio()` scales the capture by `window.devicePixelRatio` (clamped 1–4) so the image comes out the same size the table appears on screen. Chrome folds page zoom into `devicePixelRatio`, so zooming in yields a proportionally larger export — don't hardcode a pixel ratio here.
